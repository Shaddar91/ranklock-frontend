---
title: "Apollo guide: the duelist who has to close the game early"
description: "Play Apollo for the first 30 minutes and spend them making picks. What to level, when Riposte is a read instead of a button, and the patch line that quietly hit his second-favourite item."
hero: Apollo
heroSlug: apollo
heroId: 77
patchLabel: 2026-08-22 Update
pubDate: 2026-09-08
author: RankLock team
roles: [damage, offensive]
counters:
  - Victor
  - Graves
  - Seven
  - McGinnis
  - Kelvin
  - "Mo & Krill"
synergyItems: [Mystic Reverb, Spirit Burn, Escalating Exposure, Witchmail, Restorative Locket]
buildIds: [419436, 458666, 467802, 289301, 448546]
sources:
  - https://dramalock.org/guides/apollo-deadlock-guide/
  - https://api.ranklock.app/heroes/77/matchups
  - https://api.ranklock.app/heroes/77/item-win-rates
heroImage: "https://assets-bucket.deadlock-api.com/assets-api-res/images/heroes/fencer_card.png"
liveHeroes: [Apollo, Victor, Graves, Seven]
liveItems: [Mystic Reverb, Spirit Burn, Escalating Exposure, Witchmail, Restorative Locket]
---

## Who plays Apollo, and why

Pick Apollo when you want to decide the game before the enemy carry can afford the slots that beat you, and play every minute of it on that clock. He wins 52.4% of matches that end under 30 minutes, 49.5% of those running 30 to 45 and 49.2% of anything longer, a slide of 3.2 points from his best window to his worst. He has to convert a good lane into somebody's death before the game reaches the slots he cannot win in.

He is built for it. Flawless Advance is a held lunge that pays out most on a clean release, Riposte deflects the next attack and then dash-stuns a hero you choose, Disengaging Sigil leaps you backwards off an explosion that damages and slows what you left behind, and Itani Lo Sahn freezes a target on a long slash before detonating. On 770 HP and 7.2 m/s he is fast and thin, so every entry has to have an exit already planned.

## How to play Apollo as a Damage carry

Farm to Mystic Reverb and then start hunting. It is his single most distinctive purchase, bought 3.7 times more often on him than across the field, and it wins 59.4% of the 1,492,066 Apollo games it appears in, the best rate on his table by a clear margin. Spirit Burn follows at 57.4% over 2,617,469 games and Escalating Exposure at 57.4% over 1,202,343, so the spine of the damage build is spirit.

Your damage comes out in one committed sequence, which means the target selection matters more than the aim. Open on somebody standing away from their own team, hold Flawless Advance for the full release, and finish with Itani Lo Sahn on a target that is already frozen in place by the slash. If the enemy support is close enough to peel, pick someone else. The most-favourited weekly build, build 419436 at 7,452 weekly favourites, levels Flawless Advance first for exactly this reason.

## How to play Apollo as an Offensive initiator

Start the fight from a flank and press Riposte only on a read. It deflects the next attack, so the way it wins fights is by being active at the moment a predictable ability lands: a charge, a hook, a channelled ultimate. Fire it too early and the recovery is the window your opponent wanted. The second and third weekly builds, build 458666 at 6,931 favourites and `data | Absolute Demon` at 4,953, both level Disengaging Sigil first, which is the more forgiving opening if you plan to enter fights before you have items.

Enter with the Sigil pointed at your escape route. The leap travels backwards off the explosion, so placing it correctly turns a dive into a two-way trip. Against a team with more than one source of damage, the read you are making is worth less than the ground you are standing on, since a defence aimed at one attacker does nothing about crossfire.

## The build on the 2026-08-22 Update patch

Apollo was not named in this patch, but he was hit by it. Restorative Locket lost Spirit Resistance, from 10% down to 8%, and it is his second most distinctive buy at 3.6 times the roster rate and it sits in 23 of his 40 sampled builds. Radiant Regeneration lost heal on cast from 70 to 65 and appears in 13 of those 40. More than half of the published Apollo builds therefore carry at least one item that is worth slightly less than it was before 2026-08-22.

Take your item order from a list that has been touched since. `TOP 1 FENCER NA - SHMUCK`, build 448546 at 30,427 favourites, was updated 2026-09-05. `New Apollo Build` at 30,671 favourites was last updated 2026-03-15 and reads like the older item economy it was written in.

## Abilities: what to level and when

Two orders are in circulation and both end the same way. The leading weekly build goes Flawless Advance, then Disengaging Sigil, then Riposte, then Itani Lo Sahn. Every other build in the top five, weekly and lifetime, goes Disengaging Sigil, then Flawless Advance, then Riposte, then Itani Lo Sahn.

Choose on how your lane is going. Flawless Advance first is the greedy line, raising your damage while you are still trading for the wave, and it pays off when you are winning the last hits. Disengaging Sigil first is the safe line and the one four of the five published builds pick, because it gives you an escape before you have the items to survive a mistake. Riposte is third either way, since its value comes from your reads and not from its rank, and Itani Lo Sahn goes in whenever it unlocks.

## Hard counters, and how to play around them

Three heroes beat Apollo by almost exactly the same margin, and every table names the same three. He wins 43.8% of 222,386 matches against Victor, 43.8% of 268,510 against Graves and 43.8% of 275,667 against Seven. The counter endpoint puts Graves worst at 43.1% over 27,419 games, then Seven at 46.3% over 23,016 and Mo & Krill at 47.1% over 21,417. McGinnis at 45.8% over 135,269 matches and Kelvin at 45.9% over 126,327 are the next two.

What they share is that none of them offers you the single readable attack Riposte wants. Against that group, stop trying to duel and play for the objective instead: push the wave, take the boxes on your side and let a teammate be the one who eats the opening. The lanes that pay are Sinclair at 54.4% over 111,152 matches, Venator at 53.7% over 290,620, Silver at 53.2% over 204,761, Bebop at 52.7% over 307,373 and Mina at 52.6% over 292,301. In those five, back your read and take the duel.

## Items that go well

Mystic Reverb first, then whichever of Spirit Burn or Escalating Exposure the fight length calls for. Witchmail wins 57.3% of 583,805 Apollo games and Scourge 56.7% of 348,358, and Colossus at 57.7% over 69,738 is the buy when you are being forced into fights you cannot leave.

The rest of the table is smaller and more situational. Berserker sits at 56.5% over 56,890 games, Echo Shard at 56.1% over 84,753 and Spellslinger at 56.4% over 28,164, and Diviner's Kevlar at 56.5% over 28,720 is the answer to a team whose damage arrives as spirit.
