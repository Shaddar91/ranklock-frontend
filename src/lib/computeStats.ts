...
export const STAT_DEFS: StatDef[] = [
  ...
  { key: 'ammo', label: 'Clip', category: 'weapon', unit: 'flat', scope: 'global', addProps: [`${M}AMMO_CLIP_SIZE`], pctProps: [`${M}AMMO_CLIP_SIZE_PERCENT`] },
  { key: 'bullet_lifesteal', label: 'Bullet lifesteal', category: 'weapon', unit: 'percent', scope: 'global', bonusProps: [`${M}BULLET_LIFESTEAL`] },
  { key: 'reload_speed', label: 'Reload speed', category: 'weapon', unit: 'percent', scope: 'global', bonusProps: [`${M}RELOAD_SPEED`] },
  //Vitality
...
function weaponDerivation(baseline: WeaponBaseline, accOf: (key: string) => Acc, bulletFromLevel: number): WeaponDps | null {
  ...
  return {
    ...
    rows: [
      ...
      { key: 'clip_size', label: 'Clip', unit: 'flat', base: clipSize, value: clip },
      { key: 'reload_time', label: 'Reload', unit: 'seconds', base: reloadTime, value: reload },
