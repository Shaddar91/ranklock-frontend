...
function MoverRow({ s }: { s: PatchHeroStat }) {
  return (
    <div className="flex" style={{ alignItems: 'center', gap: 10, padding: '7px 0', borderTop: '1px solid var(--border)' }}>
      ...
      <a href={heroPath(s.hero_name)} className="display" style={{ flex: 1, minWidth: 0, fontWeight: 600, color: 'var(--text)', textDecoration: 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {s.hero_name}
      </a>
    ...
    () => [
      {
        ...
        render: (h) => (
          <a className="flex" href={heroPath(h.hero_name)} style={{ alignItems: 'center', gap: 10, textDecoration: 'none' }}>
