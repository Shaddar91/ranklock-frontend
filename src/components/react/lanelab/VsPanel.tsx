...
const show = (v: number, fmt: Fmt): string =>
  fmt === 'int' ? count(Math.round(v)) : fixed(v, 2);
...
export default function VsPanel({ roster, matchMode, horizonLabel }: VsPanelProps) {
    ...
    <section className="ll-vs">
      <header className="ll-sc-head">
        ...
        <p className="ll-sc-note">
          Per-game averages · {matchMode} · every game, not the window above · through{' '}
          {horizonLabel} · deltas against {firstName}
        </p>
      </header>
      ...
      <div className="ll-vs-rows">
        {roster.map((p, i) => {
          ...
          const noGames = econ != null && matches === 0;
          return (
            <div className="ll-vs-row" key={p.account_id}>
              ...
              {noGames ? (
                <p className="ll-empty">
                  No {matchMode} games on this account. Switch the bar to {otherMode(matchMode)} to
                  see {p.name}.
                </p>
              ) : (
